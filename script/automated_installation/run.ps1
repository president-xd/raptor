# ─────────────────────────────────────────────────────────────────────
#  RAPTOR | Alternative Installation Startup Script
#  Starts infrastructure via Docker, then runs backend + frontend locally
#  Uses the SAME ports as the full Docker deployment:
...
Start-Process -FilePath "python" -ArgumentList "-m uvicorn main:app --host 0.0.0.0 --port 8000" -WorkingDirectory "d:\raptor\backend" -WindowStyle Normal

# ── 4. Frontend Dashboard ───────────────────────────────────────────
Write-Host "[4/4] Starting Frontend Dashboard (Vite on :3100)..." -ForegroundColor Yellow
Start-Process -FilePath "npm" -ArgumentList "run dev" -WorkingDirectory "d:\raptor\frontend" -WindowStyle Normal
...
Write-Host "" -ForegroundColor White
Write-Host "  Mock Data:  d:\raptor\data\mock\apt29_campaign.json" -ForegroundColor DarkGray
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
